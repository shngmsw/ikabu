...
module.exports = function handleRecruit(msg) {
                ...
                if (teamId === "a") {
                    if (strCmd.match("〆")) {
                        ...
                        msg.guild.channels.cache
                            .find(channel => channel.name === "ナワバリ・フェス募集")
                            .send(getCloseEmbed(msg));
                    } else {
                        // let txt = '@everyone 【フェス募集：ヒメ派】\n' + `<@${msg.author.id}>` + 'たんがフェスメン募集中でし！\n'
                        ...
                        msg.guild.channels.cache.find(channel => channel.name === "ナワバリ・フェス募集").send(txt, {
                            embed: {
                                color: color,
                                author: {
                                    name: title,
    ...
    if (msg.content.startsWith("next") && msg.channel.name != "botコマンド") {
        ...
        if (strCmd.match("〆")) {
            msg.react("👌");
            msg.guild.channels.cache
                .find(channel => channel.name === "リグマ募集")
                .send(getCloseEmbed(msg));
        } else {
    ...
    if (msg.content.startsWith("run")) {
        ...
        if (strCmd.match("〆")) {
            msg.react("👌");
            msg.guild.channels.cache
                .find(channel => channel.name === "サーモン募集")
                .send(getCloseEmbed(msg));
        } else {
            request.get("https://splatoon2.ink/data/coop-schedules.json", function (
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    msg.guild.channels.cache.find(channel => channel.name === "サーモン募集").send(txt, {
                        embed: {
                            author: {
                                name: "SALMON RUN",
    ...
    if (msg.content.startsWith("fn")) {
        ...
        if (args[0] == "〆") {
            ...
            const role_id = msg.guild.roles.cache.find(role => role.name === "建築士");
            let txt =
                role_id.toString() +
                " 【Fortnite募集】\n" +
                `<@${msg.author.id}>` +
    ...
    if (msg.content.startsWith("mk")) {
        ...
        if (args[0] == "〆") {
            ...
            const role_id = msg.guild.roles.cache.find(role => role.name === "走り屋");
            let txt =
                role_id.toString() +
                "  【マリオカート募集】\n" +
                `<@${msg.author.id}>` +
    ...
    if (msg.content.startsWith("oc")) {
        ...
        if (args[0] == "〆") {
            ...
            const role_id = msg.guild.roles.cache.find(role => role.name === "シェフ");
            let txt =
                role_id.toString() +
                " 【オーバークック2募集】\n" +
                `<@${msg.author.id}>` +
    ...
    if (msg.content.startsWith("sb")) {
        ...
        if (args[0] == "〆") {
            ...
            const role_id = msg.guild.roles.cache.find(role => role.name === "ファイター");
            let txt =
                role_id.toString() +
                " 【スマブラSP募集】\n" +
                `<@${msg.author.id}>` +
...
function sendLeagueMatch(msg, txt, l_args) {
    ...
    msg.guild.channels.cache.find(channel => channel.name === "リグマ募集").send(txt, {
        embed: {
            author: {
                name: "リーグマッチ",
