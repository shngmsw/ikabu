...
module.exports = {
    init: async function suggestionBox(msg, user) {
        if (msg.member.permissions.has('ADMINISTRATOR') && suggestionChannelParentId == msg.channel.parent.id && msg.content === '!init') {
            ...
            msg.channel.send({ embeds: [messgeEmbed] }).then((sentMessage) => sentMessage.react('📭'));
            msg.delete();
        }
    },
    ...
    archive: async function suggestionBoxArchive(msg) {
        if (msg.member.permissions.has('ADMINISTRATOR') && suggestionChannelParentId == msg.channel.parent.id && msg.content === '!close') {
            msg.delete();
