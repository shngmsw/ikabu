...
async function main(message) {
  ...
  const yomiage = (obj) => {
    if (
      obj.cons &&
      obj.cons.status === 0 &&
      message.guild.id === context.channel.guild.id
    ) {
      mode_api(obj)
        .then((buffer) => {
  ...
  if (message.content.indexOf(`${prefix}mode`) === 0) {
    ...
    if (1 < split.length) {
      if (modeList1[split[1]] != null) {
        ...
        mode = Number(split[1]);
        message.reply(
          `指定されたAPIが不正でし指定可能なAPIは${prefix}modeで見ることが可能でし`
        );
      }
    } else {
    ...
    if (mode === 1) {
      if (1 < split.length) {
        if (voiceLists1[split[1]] != null) {
          ...
          message.reply(
            `指定された読み上げ音声タイプが不正でし指定可能な音声タイプは${prefix}typeで見ることが可能でし`
          );
        }
      } else {
        message.reply(
    ...
    if (mode === 1) {
      if (1 < split.length) {
        if (split[1] <= 200 && split[1] >= 50) {
          ...
          message.reply(
            "読み上げ音声の高さは 50 ～ 200 の範囲内で設定してほしいでし"
          );
        }
      }
    }
    ...
    if (mode === 1) {
      if (1 < split.length) {
        if (split[1] <= 200 && split[1] >= 50) {
          ...
          message.reply(
            "読み上げ音声の速度は 50 ～ 200 の範囲内で設定してほしいでし"
          );
        }
      }
    }
  ...
  const yomiage_message = await mention_replace(
    w_replace(emoji_delete(url_delete(`${message.content}`)))
  );

  if (
    !(
      isBot() ||
      isBlackListsFromID(message.member.id) ||
      isBlackListsFromPrefixes(message.content)
    ) &&
