    ...
    init: async function suggestionBox(msg, user) {
        if (msg.member.hasPermission("ADMINISTRATOR")
            ...
            let messgeEmbed = new Discord.MessageEmbed()
                ...
                .setColor(0x008080)
                .setDescription('イカ部に関するご意見や、メンバーからの嫌がらせなどを通報するにはこのメッセージに📭リアクションをしてください。\n' +
            ...
            msg.channel.send(messgeEmbed).then((sentMessage) =>
                sentMessage.react('📭'));
            msg.delete();
        }
    },
    ...
    archive: async function suggestionBoxArchive(msg) {
        if (msg.member.hasPermission("ADMINISTRATOR")
            ...
            const sendChannel = await guild.channels.cache.find(
                (channel) => channel.id === process.env.CHANNEL_ID_SUGGESTION_BOX_ARCHIVE
            );
            messages.map(
