...
export class TotalReactionsService {
  ...
  static async save(emoji_id: $TSFixMe, emoji_name: $TSFixMe, count: $TSFixMe) {
    try {
      DBCommon.init();
      await DBCommon.run(
        `insert or replace into total_reactions (emoji_id, emoji_name, count)  values ($1, $2, $3)`,
        [emoji_id, emoji_name, count]
      );
      DBCommon.close();
    } catch (err) {
      logger.error(err);
  ...
  static async update(reaction_seq: $TSFixMe, count: $TSFixMe) {
    try {
      DBCommon.init();
      await DBCommon.run(
        `update total_reactions set count = ${count}  where reaction_seq = ${reaction_seq}`
      );
      DBCommon.close();
    } catch (err) {
      logger.error(err);
