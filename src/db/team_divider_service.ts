...
module.exports = class TeamDividerService {
    ...
    static async registeredMembersStrings(message_id: $TSFixMe) {
        ...
        return new Promise((resolve, reject) => {
            db.serialize(() => {
                db.all(`SELECT * FROM team_divider where message_id = ? and match_num = 0 order by created_at`, message_id, (err: $TSFixMe, rows: $TSFixMe) => {
                    ...
                    try {
                        ...
                        logger.error(err);
                        // @ts-expect-error TS(2552): Cannot find name 'interaction'. Did you mean 'onIn... Remove this comment to see the full error message
     ...
     * @returns 取得結果
     */
    static async selectMemberFromDB(message_id: $TSFixMe, match_num: $TSFixMe, member_id: $TSFixMe) {
        const db = DBCommon.open();
        const result: $TSFixMe = [];
        return new Promise((resolve, reject) => {
            db.serialize(() => {
     ...
     * @returns 取得結果
     */
    static async selectAllMemberFromDB(message_id: $TSFixMe, match_num: $TSFixMe) {
        const db = DBCommon.open();
        const result: $TSFixMe = [];
        return new Promise((resolve, reject) => {
            db.serialize(() => {
                db.all(`SELECT * FROM team_divider where message_id = ? and match_num = ?`, message_id, match_num, (err: $TSFixMe, rows: $TSFixMe) => {
                    if (err) return reject(err);
                    rows.forEach((row: $TSFixMe) => {
                        result.push(
                            new TeamDivider(
     ...
     * @returns 取得結果
     */
    static async getTeamMember(message_id: $TSFixMe, match_num: $TSFixMe, team: $TSFixMe) {
        const result: $TSFixMe = [];
        const db = DBCommon.open();
        return new Promise((resolve, reject) => {
            db.serialize(() => {
     ...
     * @returns 取得結果
     */
    static async getForceSpectate(message_id: $TSFixMe, match_num: $TSFixMe) {
        const result: $TSFixMe = [];
        const db = DBCommon.open();
        return new Promise((resolve, reject) => {
            db.serialize(() => {
     ...
     * @returns 取得結果
     */
    static async getParticipants(message_id: $TSFixMe, match_num: $TSFixMe, team_num: $TSFixMe) {
        const result: $TSFixMe = [];
        const db = DBCommon.open();
        return new Promise((resolve, reject) => {
            db.serialize(() => {
