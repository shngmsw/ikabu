...
export class RecruitService {
  ...
  static async save(
    ...
    try {
      ...
      await DBCommon.run(
        ...
        [message_id, author_id, member_id]
      );
      DBCommon.close();
    } catch (err) {
      logger.error(err);
  ...
  static async deleteByMessageId(message_id: $TSFixMe) {
    try {
      DBCommon.init();
      await DBCommon.run(
        `DELETE from recruit where message_id = ${message_id}`
      );
      DBCommon.close();
    } catch (err) {
      logger.error(err);
  ...
  static async deleteByMemberId(message_id: $TSFixMe, member_id: $TSFixMe) {
    try {
      DBCommon.init();
      await DBCommon.run(
        `DELETE from recruit where message_id = ${message_id} and member_id = ${member_id}`
      );
      DBCommon.close();
    } catch (err) {
      logger.error(err);
  ...
  static async getRecruitAllByMessageId(message_id: string) {
    ...
    let results = await db.all(
      `select * from recruit where message_id = ${message_id} order by created_at`
    );
    DBCommon.close();
    return results;
  }
  ...
  static async getRecruitMessageByMemberId(
    message_id: string,
    member_id: string
  ) {
    const db = DBCommon.open();
    db.all = util.promisify(db.all);
    let results = await db.all(
      `select message_id from recruit where message_id = ${message_id} and member_id =${member_id}`
    );
    DBCommon.close();
    return results;
  }
