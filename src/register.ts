...
const show = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand: $TSFixMe) =>
    subcommand
      .setName("now")
      .setDescription("現在のX,バンカラマッチのステージ情報を表示")
  )
  .addSubcommand((subcommand: $TSFixMe) =>
    subcommand
      .setName("next")
      .setDescription("次のX,バンカラマッチのステージ情報を表示")
  )
  .addSubcommand((subcommand: $TSFixMe) =>
    subcommand
      .setName("nawabari")
      .setDescription("現在のナワバリのステージ情報を表示")
  )
  .addSubcommand((subcommand: $TSFixMe) =>
...
const ban = new SlashCommandBuilder()
  ...
  .addUserOption((option: $TSFixMe) =>
    option
      .setName("ban対象")
      .setDescription("banする人を指定してください。")
      .setRequired(true)
  )
  .addStringOption((option: $TSFixMe) =>
    option
...
const privateMatch = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand: $TSFixMe) =>
    subcommand
      .setName("button")
      .setDescription(
        "募集条件を通常のチャットで打ち込んだ後に通知と募集用のボタンを出せます。※@everyoneメンションを使用します。"
      )
  );
...
const variablesSettings = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand: $TSFixMe) =>
    subcommand
      .setName("表示")
      .setDescription("環境変数ファイル(.env)の設定内容を表示します。")
  )
  .addSubcommand((subcommand: $TSFixMe) =>
    subcommand
      .setName("登録更新")
...
export async function registerSlashCommands() {
  ...
  if (mode === "guild") {
    ...
    await rest
      ...
      .then(() =>
        logger.info("Successfully registered application guild commands.")
      )
      .catch((error: $TSFixMe) => {
        logger.error(error);
      });
    ...
    await rest
      ...
      .then(() =>
        logger.info("Successfully deleted application guild commands.")
      )
      .catch((error: $TSFixMe) => {
        logger.error(error);
      });
    await rest
      ...
      .then(() =>
        logger.info("Successfully registered application global commands.")
      )
      .catch((error: $TSFixMe) => {
        logger.error(error);
      });
