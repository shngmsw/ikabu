...
async function getReactionCount(
  ...
  let count = 1;
  const result = await ReactionsService.getReactionCountByPK(
    user_id,
    reaction_seq,
    channel_id,
    year
...
async function getTotalReaction(emoji_id: $TSFixMe, emoji_name: $TSFixMe) {
  let result = await TotalReactionsService.getTotalReactionByEmoji(
    emoji_id,
    emoji_name
  );
  if (isEmpty(result)) {
    await TotalReactionsService.save(emoji_id, emoji_name, 0);
    result = await TotalReactionsService.getTotalReactionByEmoji(
      emoji_id,
      emoji_name
    );
  }
