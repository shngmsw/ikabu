...
export async function guildMemberAddEvent(member: $TSFixMe) {
  try {
    ...
    const lobby_channel = await searchChannelById(
      guild,
      process.env.CHANNEL_ID_ROBBY
    );
    const beginnerRole = await searchRoleById(
      guild,
      process.env.ROOKIE_ROLE_ID
    );
    ...
    if (isEmpty(beginnerRole)) {
      ...
      const friendCode = await FriendCodeService.getFriendCodeByUserId(
        member.id
      );
      await sleep(600);
      await setRookieRole(
...
async function setRookieRole(
  guild: $TSFixMe,
  member: $TSFixMe,
  beginnerRole: $TSFixMe,
  messageCount: $TSFixMe,
  friendCode: $TSFixMe
) {
