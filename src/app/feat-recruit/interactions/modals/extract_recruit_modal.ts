...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'sendRegula... Remove this comment to see the full error message
...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'sendSalmon... Remove this comment to see the full error message
const { sendSalmonRun } = require('./salmon_recruit_modal');
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'sendFesMat... Remove this comment to see the full error message
...
async function modalRegularRecruit(interaction: $TSFixMe, params: $TSFixMe) {
...
async function modalAnarchyRecruit(interaction: $TSFixMe, params: $TSFixMe) {
    const guild = await interaction.guild.fetch();
    const channel = interaction.channel;
    const recruit_num = Number(interaction.fields.getTextInputValue('rNum'));
    let condition = interaction.fields.getTextInputValue('condition');
    ...
    if (isNotEmpty(user1)) {
        // ユーザータグからメンバー取得
        let member = members.find((member: $TSFixMe) => member.user.tag === user1);
    ...
    if (isNotEmpty(user2)) {
        // ユーザータグからメンバー取得
        let member = members.find((member: $TSFixMe) => member.user.tag === user2);
        if (isNotEmpty(member)) {
            user2 = member;
...
async function modalSalmonRecruit(interaction: $TSFixMe) {
    const guild = await interaction.guild.fetch();
    const channel = interaction.channel;
    const recruit_num = Number(interaction.fields.getTextInputValue('rNum'));
    let condition = interaction.fields.getTextInputValue('condition');
...
async function modalFesRecruit(interaction: $TSFixMe, params: $TSFixMe) {
    const guild = await interaction.guild.fetch();
    const channel = interaction.channel;
    const recruit_num = Number(interaction.fields.getTextInputValue('rNum'));
    let condition = interaction.fields.getTextInputValue('condition');
