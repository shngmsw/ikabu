...
export async function modalRegularRecruit(
  interaction: $TSFixMe,
  params: $TSFixMe
) {
  ...
  try {
    ...
    if (checkFes(data.schedule, type)) {
      const fes_channel1_id = await searchChannelIdByName(
        guild,
        "フウカ募集",
        ChannelType.GuildText,
        null
      );
      const fes_channel2_id = await searchChannelIdByName(
        guild,
        "ウツホ募集",
        ChannelType.GuildText,
        null
      );
      const fes_channel3_id = await searchChannelIdByName(
        guild,
        "マンタロー募集",
        ChannelType.GuildText,
        null
...
export async function modalAnarchyRecruit(
  interaction: $TSFixMe,
  params: $TSFixMe
) {
  const guild = await interaction.guild.fetch();
  ...
  try {
    ...
    if (isNotEmpty(user1)) {
      ...
      let member = members.find(
        (member: $TSFixMe) => member.user.tag === user1
      );
      if (isNotEmpty(member)) {
        user1 = member;
        user1_mention = `<@${member.user.id}>`;
    ...
    if (isNotEmpty(user2)) {
      // ユーザータグからメンバー取得
      let member = members.find(
        (member: $TSFixMe) => member.user.tag === user2
      );
      if (isNotEmpty(member)) {
        user2 = member;
        user2_mention = `<@${member.user.id}>`;
