 ...
 * @returns
 */
export async function handleError(err: $TSFixMe, { interaction }: $TSFixMe) {
 ...
 * @returns
 */
export async function join(interaction: $TSFixMe, params: $TSFixMe) {
  ...
  try {
    ...
    const image1_message = await searchMessageById(
      guild,
    ...
    if (participants.includes(member.user.id)) {
      ...
      // 参加済みかチェック
      const member_data = await RecruitService.getRecruitMessageByMemberId(
      ...
      try {
        if (
          isNotEmpty(host_guild_member.voice.channel) &&
          host_guild_member.voice.channel.type === ChannelType.GuildVoice
        ) {
          const host_joined_vc = await searchChannelById(
            guild,
            host_guild_member.voice.channelId
          );
 ...
 * @returns
 */
export async function cancel(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
  try {
    await interaction.update({
      components: await setButtonDisable(interaction.message, interaction),
    ...
    if (participants.includes(member.user.id)) {
      ...
      if (member_data.length > 0) {
        ...
        await RecruitService.deleteByMemberId(
          interaction.message.id,
...
export async function del(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
  try {
    // 処理待ち
    ...
    const cmd_message = await searchMessageById(
      guild,
      interaction.channelId,
      msg_id
    );
    ...
    if (isNotEmpty(image2_msg_id)) {
      image2_message = await searchMessageById(
        guild,
        interaction.channelId,
        image2_msg_id
      );
    ...
    if (participants.includes(member.user.id)) {
      ...
      try {
        ...
        logger.warn(
          "recruit message or recruit image has already been deleted"
        );
      }
      ...
      try {
        ...
        logger.warn(
          "rule image is either not set or has already been deleted."
        );
      }
 ...
 * @returns
 */
export async function close(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
  try {
    await interaction.update({
      components: await setButtonDisable(interaction.message, interaction),
    ...
    if (participants.includes(member.user.id)) {
      ...
      await interaction.channel.send({
        embeds: [helpEmbed],
        components: [createNewRecruitButton(image1_message.channel.name)],
      });
...
export async function joinNotify(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
  try {
    await interaction.update({
      components: await setButtonDisable(interaction.message, interaction),
...
export async function cancelNotify(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
  try {
    await interaction.update({
      components: await setButtonDisable(interaction.message, interaction),
...
export async function closeNotify(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
  try {
    await interaction.update({
      components: await setButtonDisable(interaction.message, interaction),
...
export async function unlock(interaction: $TSFixMe, params: $TSFixMe) {
  /** @type {Discord.Snowflake} */
...
function channelLinkButtons(guildId: $TSFixMe, channelId: $TSFixMe) {
...
export function getMemberMentions(members: $TSFixMe) {
...
async function memberListMessage(interaction: $TSFixMe) {
  const recruit_data = await RecruitService.getRecruitAllByMessageId(
    interaction.message.id
  );
  const member_list = getMemberMentions(recruit_data);
