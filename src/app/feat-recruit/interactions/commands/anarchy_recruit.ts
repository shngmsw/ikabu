...
export async function anarchyRecruit(interaction: $TSFixMe) {
  ...
  try {
    ...
    if (checkFes(data.schedule, type)) {
      const fes_channel_id = await searchChannelIdByName(
        guild,
        "フェス募集",
...
async function sendAnarchyMatch(
  ...
  const rule = new AttachmentBuilder(
    await ruleAnarchyCanvas(anarchy_data, thumbnail),
    { name: "rules.png" }
  );
