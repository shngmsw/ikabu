...
export async function recruitBigRunCanvas(
  ...
  try {
    ...
    let big_run_logo = await Canvas.loadImage(
    ...
    for (let i = 0; i < 4; i++) {
      if (count >= i + 2) {
        let user_url =
          member_urls[i] != null ? member_urls[i] : blank_avatar_url;
...
export async function ruleBigRunCanvas(data: $TSFixMe) {
  try {
    ...
    const datetime =
      formatDatetime(salmon_data.startTime, dateformat.mdwhm) +
      " - " +
      formatDatetime(salmon_data.endTime, dateformat.mdwhm);
    ...
    let date_width = rule_ctx.measureText(datetime).width;
    fillTextWithStroke(
      rule_ctx,
      datetime,
      "37px Splatfont",
    ...
    let stage_width = rule_ctx.measureText(salmon_data.stage).width;
    fillTextWithStroke(
      rule_ctx,
      salmon_data.stage,
      "38px Splatfont",
      ...
      "#2D3130",
      1,
      110 + (700 - stage_width) / 2,
      235
    );

    let illust = await Canvas.loadImage(
