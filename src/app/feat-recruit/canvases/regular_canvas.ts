...
export async function recruitRegularCanvas(
  ...
  let regular_icon = await Canvas.loadImage(
  ...
  // 募集主の画像
  let host_img = await Canvas.loadImage(
    host_member.displayAvatarURL({ extension: "png" })
  );
  recruit_ctx.save();
 ...
 */
export async function ruleRegularCanvas(regular_data: $TSFixMe) {
  const ruleCanvas = Canvas.createCanvas(720, 550);

  const date = formatDatetime(regular_data.startTime, dateformat.ymdw);
  const time =
    formatDatetime(regular_data.startTime, dateformat.hm) +
    " - " +
    formatDatetime(regular_data.endTime, dateformat.hm);
  ...
  let rule_width = rule_ctx.measureText(regular_data.rule).width;
  fillTextWithStroke(
    rule_ctx,
    regular_data.rule,
    "45px Splatfont",
  ...
  let date_width = rule_ctx.measureText(date).width;
  fillTextWithStroke(
    rule_ctx,
    date,
    "35px Splatfont",
  ...
  let time_width = rule_ctx.measureText(time).width;
  fillTextWithStroke(
    rule_ctx,
    time,
    "35px Splatfont",
  ...
  let stage1_width = rule_ctx.measureText(regular_data.stage1).width;
  fillTextWithStroke(
    rule_ctx,
    regular_data.stage1,
    "35px Splatfont",
    ...
    "#2D3130",
    1,
    (350 - stage1_width) / 2 + 10,
    440
  ); // 中央寄せ

  let stage2_width = rule_ctx.measureText(regular_data.stage2).width;
  fillTextWithStroke(
    rule_ctx,
    regular_data.stage2,
    "35px Splatfont",
