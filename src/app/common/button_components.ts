 ...
 */
export async function recoveryThinkingButton(
  interaction: $TSFixMe,
  label: $TSFixMe
) {
  const message = interaction.message;
  const newActionRow = await message.components.map(
    (oldActionRow: $TSFixMe) => {
 ...
 * @returns 新しいActionRowオブジェクト
 */
export async function disableThinkingButton(
  interaction: $TSFixMe,
  label: $TSFixMe
) {
  const message = interaction.message;
 ...
 * @returns 新しいActionRowオブジェクト
 */
export async function setButtonDisable(
  message: $TSFixMe,
  interaction?: $TSFixMe
) {
  const newActionRow = await message.components.map(
